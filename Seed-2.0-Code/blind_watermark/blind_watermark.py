...
class WaterMark:
    ...
    def extract(self, filename=None, embed_img=None, wm_shape=None, out_wm_name=None, mode='img',
                apply_attack=False, attack_type='shelter', attack_params=None, attacked_output_file=None):
        '''
        提取水印，可选择是否先对图片进行攻击
        :param filename: string，输入图片文件名
        :param embed_img: numpy array，输入图片
        :param wm_shape: tuple，水印形状
        :param out_wm_name: string，提取出的水印输出文件名（图片模式）
        :param mode: string，模式 ('img', 'str', 'bit')
        :param apply_attack: bool，是否在提取前应用攻击
        :param attack_type: string，攻击类型，目前支持 'shelter'（遮挡攻击）
        :param attack_params: dict，攻击参数字典，例如 {'ratio': 0.1, 'n': 3}
        :param attacked_output_file: string，攻击后图片的保存路径（可选）
        :return: 提取出的水印
        '''
        ...
        if apply_attack:
            if attack_type == 'shelter':
                attack_params = attack_params or {'ratio': 0.1, 'n': 3}
                embed_img = shelter_att(input_img=embed_img,
                                        ratio=attack_params.get('ratio', 0.1),
                                        n=attack_params.get('n', 3))
            if attacked_output_file:
                cv2.imwrite(attacked_output_file, embed_img)
