...
from .att import shelter_att, resize_att, bright_att, salt_pepper_att, rot_att, cut_att3
...
class WaterMark:
    ...
    def apply_attack(self, img, attack, **attack_args):
        attack_map = {
            'shelter': shelter_att,
            'resize': resize_att,
            'bright': bright_att,
        ...
        output_file = attack_args.pop('output_file', None)

        if isinstance(attack, (list, tuple)):
            for attack_config in attack:
                if isinstance(attack_config, dict):
                    attack_name = attack_config.pop('name')
                    img = self.apply_attack(img, attack_name, **attack_config)
                else:
                    img = self.apply_attack(img, attack_config)
        ...
        if attack not in attack_map:
            raise ValueError(
                ...
                f"Supported attacks: {', '.join(attack_map.keys())}"
            )

        attack_func = attack_map[attack]
        result_img = attack_func(input_img=img, **attack_args)
    ...
    def extract(self, filename=None, embed_img=None, wm_shape=None, out_wm_name=None, mode='img', 
        ...
        if attack is not None:
            embed_img = self.apply_attack(embed_img, attack, **attack_args)
