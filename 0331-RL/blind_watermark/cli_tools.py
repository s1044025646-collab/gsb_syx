...
usage2 = 'blind_watermark --extract --pwd 1234 --wm_shape 111 [--attack] embed.png'
...
optParser.add_option('--attack', dest='attack', action='store_true', default=False
                     , help='Apply shelter attack before extracting watermark')
optParser.add_option('--attack_ratio', dest='attack_ratio', type='float', default=0.1
                     , help='Ratio of each shelter block (default: 0.1)')
optParser.add_option('--attack_n', dest='attack_n', type='int', default=3
                     , help='Number of shelter blocks (default: 3)')
    ...
    if opts.work_mode == 'extract':
        if not len(args) == 1:
            ...
            wm_str = bwm1.extract(filename=args[0], wm_shape=int(opts.wm_shape), mode='str',
                                  attack=opts.attack, attack_ratio=opts.attack_ratio, attack_n=opts.attack_n)
            if opts.attack:
                print('Applied shelter attack before extraction')
